...
- 화면 흐름: 온보딩 → 채팅(빈 상태 / 대화) → 지난 대화(히스토리) → 설정, 단일 스택 내비게이션(`Navigator`)으로 구성.

## Architecture

Flutter 앱 "널널"(package name `nullnull`, Dart SDK `^3.6.1`)은 AI 여행 챗봇 서비스로, `docs/DESIGN.md`에 정의된 화면 구성(온보딩 → 채팅 → 지난 대화 → 설정)을 따른다.

- `lib/main.dart` — 앱 진입점. `AppInfo.ensureInitialized()`와 `AppThemeController.ensureInitialized()`를 초기화한 뒤 전역 `appThemeController`(`late final`)를 통해 `MaterialApp`을 `ValueListenableBuilder`로 감싸 라이트/다크 테마를 전환한다.
...
- `lib/screens/` — `onboarding_screen.dart`, `chat_screen.dart`, `history_screen.dart`, `settings_screen.dart`. 라우팅은 별도 패키지 없이 `Navigator.push(MaterialPageRoute)`로 처리한다.
- `lib/widgets/` — `app_header.dart`, `app_icon.dart`(Lucide 스타일 아이콘을 `CustomPainter`로 직접 그리는 자체 아이콘 시스템, 외부 아이콘 폰트 의존성 없음), `fade_slide_in.dart`, `chat/` 하위에 채팅 화면 전용 위젯들.
- `lib/theme/` — `app_colors.dart`(`ThemeExtension<AppColors>`로 라이트/다크 팔레트 정의, `AppColors.of(context)`로 조회), `app_text_styles.dart`, `app_theme.dart`(`AppTheme.light`/`AppTheme.dark`), `app_theme_controller.dart`(`ValueNotifier<ThemeMode>` + `shared_preferences`로 사용자의 화면 모드 선택을 영속화).
...
- `test/widget_test.dart` — 온보딩 → 채팅 → 지난 대화 → 설정으로 이동하는 위젯 테스트. `PackageInfo`/`SharedPreferences` 목 값 설정과 `AppInfo`/`AppThemeController` 초기화를 `setUpAll`에서 실제 `main()`과 동일한 순서로 재현해야 한다.
